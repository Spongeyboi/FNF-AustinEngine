...
class PlayState extends MusicBeatState
{
	...
	//Holy shit
	public static var leatherSongSpeed:Float = 1;
	public var songSpeedType:String = "multiplicative";
	public var noteKillOffset:Float = 350;

	public static var charChangeEnabled:Bool = false;
	public static var chosenBF:String = '';
	...
	public var stageGroup:FlxTypedGroup<Dynamic> = new FlxTypedGroup<Dynamic>();
	...
	public var poisonSpr:FlxSprite;
	...
	var modLoaded:Bool = false;
	var zoomHealthIcon = false;
	...
	override public function create()
	{
		...
		#if desktop
		var lemod:Null<String> = Paths.currentModDirectory;
		if (lemod != "" ||lemod != null){
			modLoaded = true;
		}
		...
		if (isStoryMode)
		{
			detailsText = "Story Mode: " + WeekData.getCurrentWeek().weekName+ '='+ (modLoaded ? Paths.currentModDirectory : 'No mod');
		}
		else
		{
			detailsText = "Freeplay"+ '='+ (modLoaded ? Paths.currentModDirectory : 'No mods');
		}
		...
		boyfriend = new Boyfriend(0, 0, (charChangeEnabled ? chosenBF : SONG.player1));
		...
		poisonSpr = new FlxSprite(FlxG.height * 0.8, FlxG.width - 75).loadGraphic(Paths.image('poison'));
		poisonSpr.scrollFactor.set();
		poisonSpr.updateHitbox();
		poisonSpr.alpha = 0;
		...
		for (folder in foldersToCheck)
		{
			if(FileSystem.exists(folder))
			{
				for (file in FileSystem.readDirectory(folder))
				{
					if(file == '.hscript' && !filesPushed.contains(file))
					{
						var expr = file;
  ...
  public function clearShaderFromCamera(cam:String){
	...
	function changeStage(?stage:String = 'Stage')
	{
		curStage = stage;
		//This function will remove the current stage and loads up a new stage.
		//NOTE: If the stage sprite is not in the stage group, it will not get removed.
		//NOTE FOR LUA: It is not recommended to use this in a stage script.
		//Use global or song script. 
		if (stageGroup != null){
			for (i in 0...stageGroup.length){
				stageGroup.members[i].destroy();
			}
		}
		switch (curStage)
		{
			...
			case 'philly': //Week 3
				...
				stageGroup.add(phillyCityLights);
				...
				stageGroup.add(street);

			case 'limo': //Week 4
				var skyBG:BGSprite = new BGSprite('limo/limoSunset', -120, -50, 0.1, 0.1);
				stageGroup.add(skyBG);

				if(!ClientPrefs.lowQuality) {
					limoMetalPole = new BGSprite('gore/metalPole', -500, 220, 0.4, 0.4);
					stageGroup.add(limoMetalPole);

					bgLimo = new BGSprite('limo/bgLimo', -150, 480, 0.4, 0.4, ['background limo pink'], true);
					stageGroup.add(bgLimo);

					limoCorpse = new BGSprite('gore/noooooo', -500, limoMetalPole.y - 130, 0.4, 0.4, ['Henchmen on rail'], true);
					stageGroup.add(limoCorpse);

					limoCorpseTwo = new BGSprite('gore/noooooo', -500, limoMetalPole.y, 0.4, 0.4, ['henchmen death'], true);
					stageGroup.add(limoCorpseTwo);

					grpLimoDancers = new FlxTypedGroup<BackgroundDancer>();
					stageGroup.add(grpLimoDancers);
					...
					stageGroup.add(limoLight);

					grpLimoParticles = new FlxTypedGroup<BGSprite>();
					stageGroup.add(grpLimoParticles);
			...
			case 'mall': //Week 5 - Cocoa, Eggnog
				...
				if(!ClientPrefs.lowQuality) {
					...
					stageGroup.add(upperBoppers);

					var bgEscalator:BGSprite = new BGSprite('christmas/bgEscalator', -1100, -600, 0.3, 0.3);
					bgEscalator.setGraphicSize(Std.int(bgEscalator.width * 0.9));
					bgEscalator.updateHitbox();
					stageGroup.add(bgEscalator);
				}
				...
				stageGroup.add(bottomBoppers);

				var fgSnow:BGSprite = new BGSprite('christmas/fgSnow', -600, 700);
				stageGroup.add(fgSnow);

				santa = new BGSprite('christmas/santa', -840, 150, 1, 1, ['santa idle in fear']);
				stageGroup.add(santa);
			...
			case 'mallEvil': //Week 5 - Winter Horrorland
				...
				stageGroup.add(evilTree);

				var evilSnow:BGSprite = new BGSprite('christmas/evilSnow', -200, 700);
				stageGroup.add(evilSnow);
			...
			case 'school': //Week 6 - Senpai, Roses
				...
				stageGroup.add(bgSky);
				bgSky.antialiasing = false;

				var repositionShit = -200;

				var bgSchool:BGSprite = new BGSprite('weeb/weebSchool', repositionShit, 0, 0.6, 0.90);
				stageGroup.add(bgSchool);
				bgSchool.antialiasing = false;

				var bgStreet:BGSprite = new BGSprite('weeb/weebStreet', repositionShit, 0, 0.95, 0.95);
				stageGroup.add(bgStreet);
		...
		//Allows lua scripts to access stage change.
		callOnLuas('onStageChange', [stage]);
	}
	...
	public function setSongTime(time:Float)
	{
		...
		@:privateAccess
		{
			lime.media.openal.AL.sourcef(FlxG.sound.music._channel.__source.__backend.handle, lime.media.openal.AL.PITCH, leatherSongSpeed);

			if (vocals.playing)
				lime.media.openal.AL.sourcef(vocals._channel.__source.__backend.handle, lime.media.openal.AL.PITCH, leatherSongSpeed);
		}
		#end
	...
	override public function update(elapsed:Float)
	{
		...
		{
			iconP1.swapOldIcon(boyfriend.curCharacter);
		}
		...
		if(ratingName == '?') {
			scoreTxt.text = 'Score: ' + songScore + ' | Misses: ' + songMisses + ' | Avg.: 0 ms | Rating: ' + ratingName + ' | (Clear) A';
		} else {
			scoreTxt.text = 'Score: ' + songScore + ' | Misses: ' + songMisses + ' | Avg.: ' + Math.round(averageMs) + ' | Rating: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)' + ' | (' + ratingFC + ') ' + letterRating;//peeps wanted no integer rating
		}
		...
		if (ClientPrefs.iconStyle == 'Ends of the health bar'){
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Change Scroll Speed':
				if (songSpeedType == "constant" || leatherSongSpeed != 1)
					return;
				var val1:Float = Std.parseFloat(value1);
				var val2:Float = Std.parseFloat(value2);
	...
	private function popUpScore(note:Note = null):Void
	{
	...
	}else if (ClientPrefs.inputSystem == 'Psych'){
		...
		if (austinJson.gameplay.comboSprite)
			if(!cpuControlled) add(comboSpr);

		currentTimingShown.screenCenter();
		currentTimingShown.x = comboSpr.x + 100;
		if (austinJson.gameplay.comboSprite){
			currentTimingShown.y = comboSpr.y + 100;
		}else currentTimingShown.y = rating.y + 100;
		//currentTimingShown.acceleration.y = 550;
		//currentTimingShown.velocity.y -= -150;
		//currentTimingShown.velocity.x -= 5;
		...
		for (i in seperatedScore)
		{
			...
			FlxTween.tween(numScore, {y: (numScore.y + FlxG.random.int(65, 100))}, 0.25, {
		...
		FlxTween.tween(currentTimingShown, {y: (currentTimingShown.y + 70)}, 0.25, {
			ease: FlxEase.circOut
		});

		FlxTween.tween(rating, {y: (rating.y + 50)}, 0.25, {
			ease: FlxEase.circOut
		});

		FlxTween.tween(comboSpr, {y: (comboSpr.y + 80)}, 0.25, {
			ease: FlxEase.circOut
		});
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if(daNote.noteType == 'Warning Note') health = 0;
			...
			if(!note.noAnimation) {
				...
				switch(note.noteType){
					...
					case 'Poison Note':
						...
						FlxG.sound.play(Paths.sound('poisonnote'), ClientPrefs.hitsoundVolume);
					case 'Hey!':
	...
	override function beatHit()
	{
		...
		if (ClientPrefs.iconStyle == 'Golden Apple Style' && curBeat % gfSpeed == 0) {
			curBeat % (gfSpeed * 2) == 0 ? {
					FlxTween.angle(iconP1, -32, 0, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.quadOut});
					...
					FlxTween.angle(iconP2, 32, 0, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.quadOut});
			...
			iconP1.updateHitbox();
			iconP2.updateHitbox();
		}else if (ClientPrefs.iconStyle == 'Austin Engine Style' && curBeat % gfSpeed == 0) {
			curBeat % (gfSpeed * 2) == 0 ? {
					FlxTween.angle(iconP1, -32, 0, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.quadOut});
					FlxTween.angle(iconP2, -32, 0, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.quadOut});
			} : {
		...
		switch(ClientPrefs.iconStyle){
			...
			case 'OS Engine Style':
				zoomHealthIcon = true;
				iconP1.angle = 16 * invert;
				iconP2.angle = 16 * invert;
				iconP1.scale.set(1.2, 1.2);
				iconP2.scale.set(1.2, 1.2);
			case 'Grow/Shrink':
				zoomHealthIcon = true;
				iconP1.scale.set((curBeat % (gfSpeed * 2) == 0  ? 1.2 : 0.6), (curBeat % (gfSpeed * 2) == 0  ? 1.2 : 0.6));
	...
	public var letterRating:String;
	public function RecalculateRating() {
		...
		if(ret != FunkinLua.Function_Stop)
		{
			...
			// Letter Rating
			letterRating = "F";
			var daRats = Math.floor(ratingPercent * 100); 
			if (daRats > 62) letterRating = 'D';
			if (daRats > 72) letterRating = 'C';
			if (daRats > 82) letterRating = 'B';
			if (daRats > 89) letterRating = 'A';
			if (daRats > 95) letterRating = 'S';
			if (daRats > 97) letterRating = 'S+';
			if (daRats == 100) letterRating = 'S++';
